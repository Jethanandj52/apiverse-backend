...
appRouter.post('/signup', async (req, res) => {
  try {
    ...
    if (!validator.isStrongPassword(password)) throw new Error("Please choose a stronger password!");
    ...
    const user = new User({ firstName, lastName, email, password: passwordHashed });
    await user.save();
...
appRouter.post("/login", async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      user: { firstName: user.firstName, lastName: user.lastName, email: user.email, _id: user._id, token }
    });
  } catch (error) {
...
appRouter.post('/forget-password', async (req, res) => {
  try {
    ...
    const frontendURL = process.env.FRONTEND_URL || "https://apiverse-frontend.vercel.app";
